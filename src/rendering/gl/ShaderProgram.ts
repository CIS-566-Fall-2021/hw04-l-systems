...
class ShaderProgram {
  ...
  attrTransform0: number; // Used in the vertex shader during instanced rendering to transform the vertex position
  attrTransform1: number; // Used in the vertex shader during instanced rendering to transform the vertex position
  attrTransform2: number; // Used in the vertex shader during instanced rendering to transform the vertex position
  attrTransform3: number; // Used in the vertex shader during instanced rendering to transform the vertex position
  attrUV: number;
  ...
  constructor(shaders: Array<Shader>) {
    ...
    this.attrTransform0 = gl.getAttribLocation(this.prog, "vs_Transform0");
  ...
  draw(d: Drawable) {
    ...
    if (this.attrTransform0 != -1 && d.bindTransform0()) {
      gl.enableVertexAttribArray(this.attrTransform0);
      gl.vertexAttribPointer(this.attrTransform0, 4, gl.FLOAT, false, 0, 0);
      gl.vertexAttribDivisor(this.attrTransform0, 1); // Advance 1 index in translate VBO for each drawn instance
    }
    ...
    if (this.attrTransform0 != -1) gl.disableVertexAttribArray(this.attrTransform0);
