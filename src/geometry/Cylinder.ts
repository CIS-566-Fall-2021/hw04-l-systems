...
class Cylinder extends Geometry {
  ...
  constructor(filepath : string = '') {
    ...
    this.m_color = vec4.fromValues(0.0, 0.0, 0.0, 1.0);
  ...
  loadMesh() {
    ...
    for(let face = 0; face < 2; face++) {
      for(let i = 0; i < subdivs; i++) {
        ...
        vec4.transformMat4(norm, norm, transMat)
        let p1 = vec4.fromValues(Math.sin(rad), 0, Math.cos(rad),1);

        let uvVec = vec2.fromValues((Math.sin(rad) + 1) * 0.5,  (Math.cos(rad) + 1) * 0.5)
        uv.push(uvVec)
    ...
    for(let i = 0; i < subdivs; i++) {
      ...
      let faceuvs = new Array<vec2>(4);
      faceuvs[0] = vec2.fromValues(uvmin, 0);
      faceuvs[1] = vec2.fromValues(uvmax, 0);
      faceuvs[2] = vec2.fromValues(uvmax, 1);
      faceuvs[3] = vec2.fromValues(uvmin, 1);
...
/*
  ...
  loadUnitCylinder()
  {
    ...
    this.face_scales = new Array<vec3>(2);
    this.face_rotates = new Array<vec3>(2);
    this.assignFaceCenters(vec3.fromValues(0.0,0.0,-0.5), vec3.fromValues(0.0, 0.0,0.5))
    this.assignFaceRotations(vec3.fromValues(90.0,0.0,0.0), vec3.fromValues(90.0, 0.0,0.0))
    this.assignFaceScales(vec3.fromValues(1.0,1.0,1.0), vec3.fromValues(1.0, 1.0,1.0))
    this.loadMesh();
